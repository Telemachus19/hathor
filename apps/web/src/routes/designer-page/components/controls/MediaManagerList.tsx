...
export function MediaManagerList({
  items = [],
  onChange,
  label = 'Media Items',
}: {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 8 }}>
      <p className={styles.propLabel}>
        {label} ({items.length})
      </p>
      {items.map((item: any, i: number) => {
        const isObj = typeof item === 'object';
        const type = isObj
          ? item.type || (isMediaVideo(item) ? 'video' : 'image')
          : isMediaVideo(item)
            ? 'video'
            : 'image';
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
              <SelField
                ...
                options={[
                  ...
                  { label: 'Video (MP4 / YT)', value: 'video' },
                ]}
              />
              <TxtInput
                value={url}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
        ...
        <button
          ...
          style={{
            padding: 6,
            border: `1px dashed ${HATHOR_ORANGE}`,
