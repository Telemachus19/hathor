...
import { Section, HATHOR_ORANGE, GREEN_ACCENT, BORDER, TEXT_PRIMARY, TEXT_MUTED, SURFACE, FONTS, WEIGHTS } from "../../../types/designerTypes";
import { PropSection, PropRow, ColorField, NumField, SelField, TxtInput, TxtArea, AlignField } from "../../controls";
...
export function TextBlockInspector({
  ...
  return (
    <PropSection title="Text Properties">
      <PropRow label="Content"><TxtArea value={targetObj.textContent || ""} onChange={v => updateTarget({ textContent: v })} rows={5} /></PropRow>
      ...
      <PropRow label="Font Size"><NumField value={targetObj.textSize || targetObj.size || 14} onChange={v => updateTarget({ textSize: v, size: v })} unit="px" min={10} max={72} /></PropRow>
      <PropRow label="Font Weight"><SelField value={targetObj.textWeight || targetObj.weight || "400"} onChange={v => updateTarget({ textWeight: v, weight: v })} options={WEIGHTS} /></PropRow>
      <PropRow label="Text Alignment"><AlignField value={targetObj.textAlign || targetObj.align || "left"} onChange={v => updateTarget({ textAlign: v as any, align: v as any })} /></PropRow>
      ...
      <p className={styles.propLabel} style={{ fontWeight: 700, color: HATHOR_ORANGE, marginTop: 12 }}>Spacing & Layout</p>
      <PropRow label="Line Height"><NumField value={targetObj.textLineHeight || 1.65} onChange={v => updateTarget({ textLineHeight: v })} step={0.05} max={3} /></PropRow>
      <PropRow label="Max Width"><NumField value={targetObj.textMaxWidth || 700} onChange={v => updateTarget({ textMaxWidth: v })} unit="px" max={1400} step={20} /></PropRow>
    </PropSection>
  );
}
...
export function HeadingBlockInspector({
  ...
  return (
    <PropSection title="Heading Properties">
      <PropRow label="Content"><TxtInput value={targetObj.text || ""} onChange={v => updateTarget({ text: v })} /></PropRow>
      <PropRow label="Font Family"><SelField value={targetObj.font || "'Cinzel', serif"} onChange={v => updateTarget({ font: v })} options={FONTS} /></PropRow>
      <PropRow label="Font Size"><NumField value={targetObj.size || 22} onChange={v => updateTarget({ size: v })} unit="px" min={12} max={72} /></PropRow>
      <PropRow label="Font Weight"><SelField value={targetObj.weight || "700"} onChange={v => updateTarget({ weight: v })} options={WEIGHTS} /></PropRow>
      <PropRow label="Color"><ColorField value={targetObj.color || "#ffffff"} onChange={v => updateTarget({ color: v })} /></PropRow>
      <PropRow label="Alignment"><AlignField value={targetObj.align || "left"} onChange={v => updateTarget({ align: v as any })} /></PropRow>
    </PropSection>
  );
}
...
export function ButtonBlockInspector({
  ...
  return (
    <PropSection title="Button Element Properties">
      <PropRow label="Button Text"><TxtInput value={targetObj.btnText || "DOWNLOAD NOW"} onChange={v => updateTarget({ btnText: v })} /></PropRow>
      <PropRow label="Button Background / Gradient"><ColorField value={targetObj.btnGradient || targetObj.btnBg || GREEN_ACCENT} onChange={v => updateTarget({ btnGradient: v, btnBg: v })} placeholder="e.g. #38d39f or linear-gradient(...)" /></PropRow>
      <PropRow label="Button Text Color"><ColorField value={targetObj.btnColor || "#0e1116"} onChange={v => updateTarget({ btnColor: v })} /></PropRow>
      <PropRow label="Border Color"><ColorField value={targetObj.btnBorderColor || "transparent"} onChange={v => updateTarget({ btnBorderColor: v })} /></PropRow>

      <p className={styles.propLabel} style={{ fontWeight: 700, color: HATHOR_ORANGE, marginTop: 12 }}>Button Sizing & Radius</p>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        <PropRow label="Pad Top/Bottom"><NumField value={targetObj.btnPaddingV ?? 12} onChange={v => updateTarget({ btnPaddingV: v })} unit="px" max={50} /></PropRow>
        <PropRow label="Pad Left/Right"><NumField value={targetObj.btnPaddingH ?? 16} onChange={v => updateTarget({ btnPaddingH: v })} unit="px" max={80} /></PropRow>
      </div>
      <PropRow label="Border Radius"><NumField value={targetObj.btnRadius ?? 3} onChange={v => updateTarget({ btnRadius: v })} unit="px" max={40} /></PropRow>
    </PropSection>
  );
}
...
export function FeaturesInspector({
  ...
  return (
    <PropSection title="Features Grid Settings">
      <PropRow label="Section Title"><TxtInput value={targetObj.featuresTitle || "KEY FEATURES"} onChange={v => updateTarget({ featuresTitle: v })} /></PropRow>
      <PropRow label="Title Font"><SelField value={targetObj.featuresTitleFont || targetObj.titleFont || "'Cinzel', serif"} onChange={v => updateTarget({ featuresTitleFont: v, titleFont: v })} options={FONTS} /></PropRow>
      <PropRow label="Title Color"><ColorField value={targetObj.featuresTitleColor || targetObj.titleColor || "#ffffff"} onChange={v => updateTarget({ featuresTitleColor: v, titleColor: v })} /></PropRow>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 12 }}>
        ...
        <button onClick={() => updateTarget({ featuresItems: [...(targetObj.featuresItems || []), { title: "NEW FEATURE", desc: "Feature description goes here.", icon: "zap", color: HATHOR_ORANGE }] })}
...
export function DividerInspector({
  ...
  return (
    <PropSection title="Divider Line Settings">
      ...
      <PropRow label="Thickness"><NumField value={targetObj.dividerThickness || 1} onChange={v => updateTarget({ dividerThickness: v })} unit="px" max={20} /></PropRow>
    </PropSection>
  );
}
...
export function SpacerInspector({
  ...
  return (
    <PropSection title="Vertical Spacer Settings">
      <PropRow label="Height"><NumField value={targetObj.spacerHeight || 30} onChange={v => updateTarget({ spacerHeight: v })} unit="px" max={300} /></PropRow>
    </PropSection>
  );
}
