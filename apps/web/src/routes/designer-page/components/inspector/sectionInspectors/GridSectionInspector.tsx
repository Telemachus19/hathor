    ...
    <>
      <PropSection title="Multi-Column Layout & Grid Ratios">
        ...
        <PropRow label="Column Gap Spacing">
          <NumField
            value={targetObj.gridGap ?? 32}
            onChange={(v) => updateTarget({ gridGap: v })}
            unit="px"
            max={100}
          />
        </PropRow>
        ...
        {currentCol && (
          <>
            <PropRow label="Column Background">
              <ColorField
                value={currentCol.bg || 'transparent'}
                onChange={(v) => updateCurrentCol({ bg: v })}
              />
            </PropRow>

            <PropRow label="Top Accent Border Line Color">
              <ColorField
                value={currentCol.borderTopColor || 'transparent'}
                onChange={(v) => updateCurrentCol({ borderTopColor: v })}
              />
            </PropRow>
            ...
            <div style={{ marginTop: 12, borderTop: `1px solid ${BORDER}`, paddingTop: 10 }}>
              <p
                ...
                style={{ fontWeight: 700, color: HATHOR_ORANGE, marginBottom: 6 }}
              >
                Add Component to Col #{gridColIdx + 1}
              </p>
            ...
            <div style={{ marginTop: 12, borderTop: `1px solid ${BORDER}`, paddingTop: 10 }}>
              <p className={styles.propLabel} style={{ fontWeight: 700, marginBottom: 6 }}>
                Column #{gridColIdx + 1} Elements ({currentCol.elements.length})
              </p>
              {currentCol.elements.length === 0 ? (
                <p style={{ fontSize: 10, color: TEXT_MUTED, fontStyle: 'italic', margin: 0 }}>
                  No components in this column yet.
                </p>
              ) : (
                      ...
                      >
                        <span
                          style={{
                            fontSize: 10,
                            fontWeight: 700,
                            color: isElSelected ? HATHOR_ORANGE : TEXT_MUTED,
                          }}
                        >
                          #{elIdx + 1} {el.type}
                        </span>
                        <div style={{ display: 'flex', gap: 2 }}>
                          ...
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              updateCurrentCol({
                                elements: currentCol.elements.filter((_, j) => j !== elIdx),
                              });
