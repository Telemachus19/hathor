...
import {
  Monitor, Tablet, Smartphone, RotateCcw, RotateCw, Save,
  Upload, LayoutGrid, FileUp, FileJson, Eye
} from "lucide-react";
...
export function DesignerHeader({
  ...
  return (
    <div className={styles.topToolbar}>
      <span style={{ color: HATHOR_ORANGE, fontWeight: 900, fontFamily: "monospace", fontSize: 13, letterSpacing: "0.1em" }}>HATHOR</span>
      ...
      <button onClick={onOpenTemplates} className={styles.saveDraftBtn} style={{ background: "transparent", border: "1px solid #353c4d", color: TEXT_MUTED }}>
      ...
      {/* Undo / Redo */}
      <button onClick={onUndo} disabled={historyIdx <= 0} className={styles.iconActionBtn} title="Undo">
        <RotateCcw size={13} />
      </button>
      <button onClick={onRedo} disabled={historyIdx >= historyLength - 1} className={styles.iconActionBtn} title="Redo">
        <RotateCw size={13} />
      </button>
      ...
      {/* Actions */}
      <button onClick={onOpenPreview} className={styles.saveDraftBtn} style={{ background: "rgba(56, 211, 159, 0.14)", border: `1px solid ${GREEN_ACCENT}`, color: GREEN_ACCENT, fontWeight: 800 }}>
        <Eye size={12} /> Preview Game Page
      </button>
      <button onClick={onOpenImport} className={styles.saveDraftBtn} style={{ background: "rgba(242, 107, 33, 0.14)", border: `1px solid ${HATHOR_ORANGE}`, color: HATHOR_ORANGE, fontWeight: 800 }}>
        <FileUp size={12} /> Import JSON
      </button>
      <button onClick={onSaveDraft} className={styles.saveDraftBtn}>
        <Save size={11} /> Save Draft
      </button>
      <button onClick={onOpenPublish} className={styles.saveDraftBtn} style={{ background: "transparent", border: `1px solid ${BORDER}`, color: TEXT_MUTED }}>
