import { Section, PageSettings, DEFAULT_PAGE_SETTINGS, SURFACE, BORDER, HATHOR_ORANGE, GREEN_ACCENT, TEXT_PRIMARY, TEXT_MUTED, createGridElement } from "../types/designerTypes";
          ...
          elements: col.elements.map(el => {
            ...
            if (el.type === "system-reqs") {
              ...
              return {
                ...el,
                reqsMin: {
                  os: (draft.minReq.os && draft.minReq.os.length > 0) ? draft.minReq.os.join(", ") : (reqEl.reqsMin?.os || "Windows 10 (64-bit)"),
                  cpu: draft.minReq.cpu || reqEl.reqsMin?.cpu || "Intel Core i5-8400",
                  ram: draft.minReq.ram ? (draft.minReq.ram.toUpperCase().includes('GB') ? draft.minReq.ram : `${draft.minReq.ram} GB`) : (reqEl.reqsMin?.ram || "12 GB"),
                  gpu: draft.minReq.gpu || reqEl.reqsMin?.gpu || "NVIDIA GTX 1070",
                  storage: draft.minReq.storageNum ? `${draft.minReq.storageNum} ${draft.minReq.storageSuffix}` : (reqEl.reqsMin?.storage?.replace(/NVMe SSD|Available Space/gi, '').trim() || "85 GB")
                },
                reqsRec: {
                  os: (draft.recReq.os && draft.recReq.os.length > 0) ? draft.recReq.os.join(", ") : (reqEl.reqsRec?.os || "Windows 11 (64-bit)"),
                  cpu: draft.recReq.cpu || reqEl.reqsRec?.cpu || "Intel Core i7-12700K",
                  ram: draft.recReq.ram ? (draft.recReq.ram.toUpperCase().includes('GB') ? draft.recReq.ram : `${draft.recReq.ram} GB`) : (reqEl.reqsRec?.ram || "16 GB"),
                  gpu: draft.recReq.gpu || reqEl.reqsRec?.gpu || "NVIDIA RTX 4070",
                  storage: draft.recReq.storageNum ? `${draft.recReq.storageNum} ${draft.recReq.storageSuffix}` : (reqEl.reqsRec?.storage?.replace(/NVMe SSD|Available Space/gi, '').trim() || "85 GB")
                }
              };
            }
            return el;
