...
export const HeadingRenderer: React.FC<{ s?: any; device?: string; pageSettings?: any }> = ({
  s = {},
  device,
  pageSettings,
}) => {
...
export const TextRenderer: React.FC<{ s?: any; device?: string; pageSettings?: any }> = ({
  s = {},
  device,
  pageSettings,
}) => {
  ...
  return (
    <div style={{ textAlign: 'center', width: '100%', boxSizing: 'border-box', marginBottom: 16 }}>
      <img
        ...
        alt={s.imageAlt || ''}
        style={{
          maxWidth: `${s.imageMaxWidth ?? 100}%`,
          width: '100%',
          display: 'inline-block',
          borderRadius: s.imageRadius || 4,
          boxShadow: s.imageShadow ? '0 24px 60px rgba(0,0,0,0.55)' : 'none',
        }}
      />
    </div>
  );
...
export const DividerRenderer: React.FC<{ s?: any }> = ({ s = {} }) => {
  return (
    <hr
      style={{
        borderColor: s.dividerColor || BORDER,
        borderWidth: s.dividerThickness || 1,
        margin: '16px 0',
        width: '100%',
      }}
    />
